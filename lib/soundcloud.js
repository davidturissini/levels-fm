...
var Soundcloud = {
	...
	resource: function (path, total, requestOptions) {
		var limit = 200, pages = Math.ceil(total / limit), results = [], i, funcs = [], options;
...
Soundcloud.User = Levels.Class({
	...
	tracks: function (options) {
		...
		return Soundcloud.resource('/users/' + this.get('permalink') + '/tracks', this.get('track_count'), options)
	...
	followings: function (options) {
		...
		return Soundcloud.resource('/users/' + this.get('permalink') + '/followings', this.get('followings_count'), options)
			.then(function (followingsJSON) {
				var followings = new Soundcloud.UsersCollection(), user = this;

				followingsJSON.forEach(function (followingJSON) {
					var following = new Soundcloud.User(followingJSON);
	...
	followers: function () {
		...
		return Soundcloud.resource('/users/' + this.get('permalink') + '/followers', this.get('followers_count'), options)
